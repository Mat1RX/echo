...
import dev.brahmkshatriya.echo.playback.MediaItemUtils.getCachedProgress
...
class PlayerUiListener(
    ...
    // 🚀 Optimization: Rate-limit disk cache polling to 1x per second to prevent 
    // ExoPlayer lock contention and massive CPU spikes on the UI thread.
    private var lastCacheCheckMs = 0L
    private var lastCachedProgress = 0f
    ...
    private fun updateProgress(caller: String = "poll") {
        ...
        // Retrieve the cached bytes from the background cache writer 
        // and calculate the true buffered duration based on the player's total duration.
        var diskCachedMs = 0L
        val currentMediaItem = viewModel.playerState.current.value?.mediaItem
        if (currentMediaItem != null && durationSet) {
            val key = metadataKey(
                currentMediaItem.track.id,
                currentMediaItem.serverIndex,
                currentMediaItem.sourceIndex,
                currentMediaItem.extensionId
            )
            // 🚀 Optimization: Only poll the disk cache once per second to avoid lock contention
            val now = System.currentTimeMillis()
            if (now - lastCacheCheckMs > 1000L) {
                lastCachedProgress = viewModel.cache.getCachedProgress(key)
                lastCacheCheckMs = now
            }
            diskCachedMs = (lastCachedProgress * playerDuration).toLong()
        }
        val trueBuffered = kotlin.math.max(buffered, diskCachedMs)
        ...
        viewModel.progress.value = displayPosition to trueBuffered
        ...
        if (state != ExoPlayer.STATE_IDLE && state != ExoPlayer.STATE_ENDED) {
            ...
            // Increase the polling rate to 100ms when actively downloading 
            // to ensure the buffering progress bar updates smoothly in the UI.
            val duration = totalDuration ?: 0L
            if (trueBuffered < duration) {
                delayMs = kotlin.math.min(delayMs, 100L)
            }
