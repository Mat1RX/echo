...
class ShufflePlayer(
    ...
    // Provider for pause/resume fade durations. 
    // Pair(pauseFadeMs, resumeFadeMs)
    private val fadeDurations: (() -> Pair<Long, Long>)? = null,
    private val skipFadeDuration: () -> Long = { 0L },
    ...
    // 🚀 Feature: Hardware Volume Fade
    // Smoothly animates player.volume to prevent residual audio pops during pause/resume
    private fun fadeVolume(from: Float, to: Float, durationMs: Long, onDone: () -> Unit) {
        fadeJob?.cancel()
        if (durationMs <= 0L) {
            player.volume = to
            onDone()
        ...
        fadeJob = CoroutineScope(Dispatchers.Main).launch {
            ...
            for (i in 0..steps) {
                ...
                // Smooth sine curve for natural volume perception
                val curve = kotlin.math.sin(progress * Math.PI / 2).toFloat()
                player.volume = from + (to - from) * curve
                delay(stepDelay)
            }
            player.volume = to
            onDone()
    ...
    fun requestFadeTransition(performSeek: Runnable) {
        val duration = skipFadeDuration()
        if (duration > 0L && player.playbackState == STATE_READY && player.playWhenReady) {
            pendingSkip?.run()
            pendingSkip = performSeek
            fadeVolume(player.volume, 0f, duration) {
                if (pendingSkip == performSeek) {
                    pendingSkip = null
                    performSeek.run()
                    player.volume = 1f
                }
    ...
    override fun play() {
        ...
        val resumeDuration = fadeDurations?.invoke()?.second ?: 0L
        if (resumeDuration > 0L) {
            // Cancel any ongoing fade-out before starting fade-in
            fadeJob?.cancel()
            player.volume = 0f
            
            // Start the fade-in animation first.
            // This prevents a race condition where super.play() triggers synchronous focus denial 
            // and pauses the player, only to be immediately overwritten by this fade-in logic.
            fadeVolume(0f, 1f, resumeDuration) {}
            super.play()
        } else {
            player.volume = 1f
            super.play()
    ...
    override fun pause() {
        val duration = fadeDurations?.invoke()?.first ?: 0L
        if (duration > 0L && player.playbackState == STATE_READY && player.playWhenReady) {
            // Start fade-out animation and pause the player only when the animation completes
            fadeVolume(player.volume, 0f, duration) {
                super.pause()
                player.volume = 1f // Reset volume so it's ready for the next play
            }
        } else {
            super.pause()
        }
    ...
    /**
     * Pauses the player immediately without any fade-out animation.
     * Crucial for AUDIOFOCUS_LOSS events, as failing to pause immediately causes Android 12+ 
     * to permanently mute the application for violating audio focus rules.
     */
    ...
    override fun setPlayWhenReady(playWhenReady: Boolean) {
        if (playWhenReady) {
            ...
            if (resumeDuration > 0L) {
                ...
                // Start the fade-in animation first to prevent synchronous pause-overwrite race conditions
                fadeVolume(0f, 1f, resumeDuration) {}
    ...
    private fun jumpForwardTo(targetIndex: Int, play: Boolean) {
        ...
        if (targetIndex <= current) {                                  // not a forward jump → plain seek
            val performSeek = Runnable { 
                super.seekTo(targetIndex, 0)
                if (play) playWhenReady = true
            }
            requestFadeTransition(performSeek)
            return
        }
    ...
    private fun advanceForward(doSeek: () -> Unit) {
        ...
        // 🚀 Optimization / Feature: Adaptive Fadeout on Skip
        // If the caller provided requestFadeTransition, and this is a normal manual skip, route it there to fade out.
        if (!skipHistory) {
            requestFadeTransition(performSeek)
        } else {
            performSeek.run()
    ...
    private fun handlePrevious() {
        ...
        if (player.currentPosition > PREVIOUS_RESTART_THRESHOLD_MS) {
            val performSeek = Runnable { player.seekToDefaultPosition() }
            requestFadeTransition(performSeek)
            return
        }
