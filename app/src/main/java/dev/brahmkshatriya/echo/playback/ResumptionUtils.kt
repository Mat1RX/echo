...
object ResumptionUtils {
    ...
    suspend fun saveQueue(context: Context, player: Player) = withContext(Dispatchers.Main) {
        ...
        Log.d("EchoPlayback", "saveQueue: itemCount=${list.size}")
        if (list.isEmpty()) {
            Log.d("EchoPlayback", "saveQueue: empty — stack: ${Thread.currentThread().stackTrace.take(10).joinToString(" < ") { it.methodName }}")
            return@withContext
        }
    ...
    private fun Context.recoverQueue(
        ...
        if (dropped > 0)
            Log.w("EchoPlayback", "recoverQueue: windowed [$current,$end) of ${tracks.size}; skipped $dropped/${window.size} unbuildable")
    ...
    fun Context.recoverPlaylist(
        ...
        Log.d("EchoPlayback", "recoverPlaylist: ${items.size} items index=0 (windowed) pos=$rawPos safePos=$safePos duration=$trackDuration")
