...
class PlayerEventListener(
    ...
    override fun onPlaybackStateChanged(playbackState: Int) {
        Log.d("EchoPlayback", "onPlaybackStateChanged: state=$playbackState")
    ...
    private fun armBufferingWatchdog() {
        Log.d("EchoPlayback", "STATE_BUFFERING: ${player.currentMediaItem?.mediaId} \"${player.currentMediaItem?.mediaMetadata?.title}\"")
        ...
        bufferingWatchdog = scope.launch {
            ...
            withContext(Dispatchers.Main) {
                ...
                if (player.currentMediaItem?.isLoaded == false
                    ...
                    Log.d("EchoPlayback", "Buffering watchdog: cold resolution in flight, re-arming")
                    armBufferingWatchdog()
                    return@withContext
                }
                ...
                if (retriedMediaId != currentMediaId) {
                    ...
                    Log.d("EchoPlayback", "Buffering watchdog: retrying $currentMediaId (attempt 1/$maxWatchdogRetries)")
                    ...
                    Log.d("EchoPlayback", "Buffering watchdog: retrying $currentMediaId (attempt $retriedWatchdogCount/$maxWatchdogRetries)")
                    val savedPosition = player.currentPosition
                    player.stop()
                    internalSeek { player.seekTo(savedPosition) }
                    player.prepare()
                    ...
                    retriedMediaId = null
                    retriedWatchdogCount = 0
                    Log.d("EchoPlayback", "Buffering watchdog fired: skipping ${player.currentMediaItem?.mediaId}")
                    recordSkip(null)
    ...
    override fun onPlayerError(error: PlaybackException) {
        ...
        if (rootCause is CancellationException && rootCause !is TimeoutCancellationException) {
            Log.d("EchoPlayback", "onPlayerError: ignoring CancellationException for ${mediaItem?.mediaId}")
            return
        }
        ...
        if (rootCause is HttpDataSource.InvalidResponseCodeException && rootCause.responseCode == 404) {
            ...
            if (retried404MediaId != currentMediaId) {
                ...
                Log.d("EchoPlayback", "onPlayerError: 404 for $currentMediaId, retrying with stop/prepare")
                ...
                retried404MediaId = null
                Log.d("EchoPlayback", "onPlayerError: 404 retry failed for $currentMediaId, skipping")
        ...
        if (isTransientServerError) {
            ...
            if (retriedSocketMediaId == null || retriedSocketMediaId != currentMediaId) {
                ...
                Log.d("EchoPlayback", "onPlayerError: SocketException for $currentMediaId, retrying")
                val savedIndex = player.currentMediaItemIndex
                val savedPosition = player.currentPosition
                player.stop()
                internalSeek { player.seekTo(savedIndex, savedPosition) }
                ...
                retriedSocketMediaId = null
                Log.d("EchoPlayback", "onPlayerError: SocketException retry failed for $currentMediaId, skipping")
                recordSkip(rootCause, error)
                if (consecutiveUnavailableSkips >= maxConsecutiveUnavailableSkips) {
                    reportAndResetConsecutiveSkips(mediaItem?.extensionId)
                    player.stop()
        ...
        if (isNetworkDown) {
            ...
            if (retriedNetworkMediaId == null || retriedNetworkMediaId != currentMediaId) {
                ...
                Log.d("EchoPlayback", "onPlayerError: network down for $currentMediaId, retrying once")
                val savedIndex = player.currentMediaItemIndex
                val savedPosition = player.currentPosition
                player.stop()
                internalSeek { player.seekTo(savedIndex, savedPosition) }
                ...
                Log.d("EchoPlayback", "onPlayerError: network down retry failed for $currentMediaId, holding")
                scope.launch { throwableFlow.emit(PlayerException(mediaItem, rootCause)) }
                player.pause()
            }
            return
        ...
        if (is401) {
            ...
            if (retriedMediaId != currentMediaId) {
                retriedMediaId = currentMediaId
                retriedWatchdogCount = 1
                Log.d("EchoPlayback", "onPlayerError: 401 for $currentMediaId, retrying with stop/prepare (fresh TRACK_TOKEN)")
                val savedIndex = player.currentMediaItemIndex
                val savedPosition = player.currentPosition
                player.stop()
                internalSeek { player.seekTo(savedIndex, savedPosition) }
            ...
            retriedMediaId = null
            retriedWatchdogCount = 0
            Log.d("EchoPlayback", "onPlayerError: 401 retry exhausted for $currentMediaId, skipping")
        ...
        if (currentRetries >= maxRetries) {
            currentRetries = 0
            last = null
            Log.d("EchoPlayback", "onPlayerError: maxRetries exhausted for ${mediaItem.mediaId}, skipping")
            recordSkip(rootCause, error)
            if (consecutiveUnavailableSkips >= maxConsecutiveUnavailableSkips) {
                reportAndResetConsecutiveSkips(mediaItem.extensionId)
                player.stop()
