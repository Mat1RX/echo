...
class PlayerService : MediaLibraryService() {
    ...
    // blank "Echo" notification on cold restart with no track loaded. START_NOT_STICKY means
    ...
    private val audioEffectsProcessor by lazy {
        AudioEffectsProcessor().apply {
            ...
            isProcessingEnabled = app.settings.getBoolean(AUDIO_PROCESSING_ENABLED, true)
    ...
    private val listener = SharedPreferences.OnSharedPreferenceChangeListener { prefs, key ->
        when (key) {
            ...
            AUDIO_PROCESSING_ENABLED -> {
                audioEffectsProcessor.isProcessingEnabled = prefs.getBoolean(key, true)
                effects.updateNormalizationSettings() // trigger a refresh of the equalizer state
            }
    ...
    override fun onCreate() {
        ...
        val fadeDurations: () -> Pair<Long, Long> = {
            Pair(
                app.settings.getInt(PAUSE_FADE_DURATION, 5) * 100L,
                app.settings.getInt(RESUME_FADE_DURATION, 5) * 100L
            )
        }
        val skipFadeDuration: () -> Long = {
            app.settings.getInt(SKIP_FADE_DURATION, 10) * 100L
        }

        val player = ShufflePlayer(exoPlayer, ::mapAaError, fadeDurations, skipFadeDuration)
        ...
        state.restoreDeferred = scope.async(Dispatchers.IO) {
            with(this@PlayerService) {
                ...
                Log.d("EchoPlayback", "restore read: items=${items.size}")
    ...
    @OptIn(UnstableApi::class)
    private fun offloadPreferences(offloadEnabled: Boolean) =
        TrackSelectionParameters.AudioOffloadPreferences.Builder()
            .setAudioOffloadMode(
                if (offloadEnabled) AUDIO_OFFLOAD_MODE_ENABLED else AUDIO_OFFLOAD_MODE_DISABLED
            )
            // Do NOT require gapless or speed-change support in offload mode.
            // Requiring speed-change (which almost no mobile DSP supports in offload) or gapless
            // causes ExoPlayer to reject offload for FLAC/AAC and fall back to CPU decoding.
    ...
    private fun createExoplayer(
        ...
        val audioOffloadPreferences = offloadPreferences(app.settings.getBoolean(AUDIO_OFFLOAD_ENABLED, false))
        ...
        ExoPlayer.Builder(this, factory)
            ...
            .setLoadControl(
                if (app.settings.getBoolean(PRELOAD_TRACK_CACHE, true)) {
                    val gaplessS = app.settings.getInt(PRELOAD_FUTURE_TRACKS_S, 10).toLong()
                    AggressiveAudioLoadControl(gaplessS * 1_000_000L)
                } else {
                    DefaultLoadControl()
                }
            )
            ...
            .also {
                ...
                if (app.settings.getBoolean(PRELOAD_TRACK_CACHE, true)) {
                    val gaplessS = app.settings.getInt(PRELOAD_FUTURE_TRACKS_S, 10).toLong()
                    it.preloadConfiguration = ExoPlayer.PreloadConfiguration(gaplessS * 1_000_000L)
                } else {
    ...
    companion object {
        ...
        private const val ACTION_CLEAR_QUEUE = "dev.brahmkshatriya.echo.CLEAR_QUEUE"
        const val SKIP_SILENCE = "skip_silence"
        const val LOUDNESS_NORMALIZATION = "loudness_normalization"
        const val AUDIO_OFFLOAD_ENABLED = "hardware_audio_offload"
        const val AUDIO_PROCESSING_ENABLED = "audio_processing_enabled"
        const val SKIP_FADE_DURATION = "skip_fade_duration"
        const val PAUSE_FADE_DURATION = "pause_fade_duration"
        const val RESUME_FADE_DURATION = "resume_fade_duration"
        ...
        const val PRELOAD_TRACK_CACHE = "preload_track_cache"
        const val PRELOAD_FUTURE_TRACKS_S = "preload_future_tracks_s"
        const val CACHE_IN_RAM_ONLY = "cache_in_ram_only"
...
@OptIn(UnstableApi::class)
private class AggressiveAudioLoadControl(
    private val gaplessBufferUs: Long
) : DefaultLoadControl(
    androidx.media3.exoplayer.upstream.DefaultAllocator(true, androidx.media3.common.C.DEFAULT_BUFFER_SEGMENT_SIZE),
    50_000, // minBufferMs
    50_000, // minBufferForLocalPlaybackMs
    1200_000, // maxBufferMs
    1200_000, // maxBufferForLocalPlaybackMs
    DEFAULT_BUFFER_FOR_PLAYBACK_MS,
    DEFAULT_BUFFER_FOR_PLAYBACK_MS,
    DEFAULT_BUFFER_FOR_PLAYBACK_AFTER_REBUFFER_MS,
    DEFAULT_BUFFER_FOR_PLAYBACK_AFTER_REBUFFER_MS,
    150 * 1024 * 1024, // targetBufferBytes
    true, // prioritizeTimeOverSizeThresholds
    true, // prioritizeTimeOverSizeThresholdsForLocalPlayback
    0, // backBufferDurationMs
    ...
    override fun shouldContinueLoading(parameters: androidx.media3.exoplayer.LoadControl.Parameters): Boolean {
        if (parameters.playbackPositionUs < 0) {
            val futureBufferedUs = parameters.bufferedDurationUs + parameters.playbackPositionUs
            if (futureBufferedUs >= gaplessBufferUs) {
                return false
            }
        }
