...
// skips visibly where the original token is required (e.g. Provider) instead of vanishing.
...
abstract class AndroidAutoCallback(
    ...
    override fun onGetLibraryRoot(
        ...
        Log.d("EchoAuto", "onGetLibraryRoot: extensionList.value.size=${extensionList.value.size}, clearing caches, starting watcher")
        extensionWatcherJob?.cancel()
        extensionWatcherJob = scope.launch {
            ...
            extensionList.collect { extensions ->
                Log.d("EchoAuto", "extensionWatcher: collected ${extensions.size} extensions: ${extensions.map { it.id }}")
                if (extensions.isNotEmpty()) {
                    Log.d("EchoAuto", "extensionWatcher: calling notifyChildrenChanged ROOT count=${extensions.size}")
    ...
    override fun onSearch(
        ...
        Log.d("EchoAuto", "onSearch: rawQuery='$query' effectiveQuery='$effectiveQuery'")
        ...
        if (existing != null && existing.isActive) {
            Log.d("EchoAuto", "onSearch: joining existing in-flight search for query='$query'")
            scope.launch {
                runCatching {
                    existing.join()
                    val tracks = searchResults[cacheKey]
                    if (tracks != null) {
                        Log.d("EchoAuto", "onSearch: notifySearchResultChanged (joined) query='$query' count=${tracks.size}")
        ...
        pendingSearchJob = scope.launch {
            ...
            runCatching { performSearch(effectiveQuery) }
                .onSuccess { tracks ->
                    ...
                    Log.d("EchoAuto", "onSearch: notifySearchResultChanged query='$query' count=${tracks.size}")
                    session.notifySearchResultChanged(browser, query, tracks.size, params)
                }
                .onFailure {
                    searchJobs.remove(query)
    ...
    override fun onGetChildren(
        ...
        if (parentId == ROOT) {
            ...
            Log.d("EchoAuto", "onGetChildren ROOT: extensionList.first size=${extensions.size} enabled=${enabled.size}, ids=${enabled.map { it.id }}")
    ...
    override fun onGetSearchResult(
        ...
        val extId = getCurrentExtension()?.id ?: ""
        val cacheKey = query to extId
        Log.d("EchoAuto", "onGetSearchResult: query='$query' page=$page pageSize=$pageSize lastSearchQuery='$lastSearchQuery' cachedResults=${searchResults[cacheKey]?.size ?: "none"}")
        return scope.future {
            ...
            Log.d("EchoAuto", "onGetSearchResult: returning ${allTracks.drop(from).take(pageSize).size} items (total=${allTracks.size}) for query='$query'")
    ...
    private suspend fun performSearch(query: String): List<MediaItem> {
        ...
        if (ext == null) {
            Log.d("EchoAuto", "performSearch: no extension available for query='$query'")
            return emptyList()
        }
        Log.d("EchoAuto", "performSearch: query='$query' ext=${ext.id}")
        return runCatching {
            withTimeout(10_000) {
                val client = ext.instance.value().getOrNull() as? SearchFeedClient
                    ?: run {
                        Log.d("EchoAuto", "performSearch: ${ext.id} has no SearchFeedClient")
                        return@withTimeout emptyList<MediaItem>()
                    }
                Log.d("EchoAuto", "performSearch: calling loadSearchFeed query='$query' ext=${ext.id}")
                ...
                Log.d("EchoAuto", "performSearch: ${tracks.size} results for query='$query' ext=${ext.id}")
            ...
            when (it) {
                is TimeoutCancellationException -> {
                    Log.d("EchoAuto", "performSearch: timeout for query='$query' ext=${ext.id}")
                    emptyList()
                }
                is CancellationException -> throw it
                else -> {
                    throwableFlow?.emit(it)
                    Log.d("EchoAuto", "performSearch: error for query='$query' ext=${ext.id}: ${it::class.simpleName}: ${it.message}")
                    emptyList()
                }
            }
        }
            ...
            if (it.mediaId.startsWith("auto/")) {
                ...
                when {
                    ...
                    auto.e != null -> {
                        ...
                        // error-skips visibly (e.g. Provider, token-less) instead of vanishing.
