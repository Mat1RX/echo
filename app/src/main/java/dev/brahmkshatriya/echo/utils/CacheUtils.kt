...
object CacheUtils {
    ...
    inline fun <reified T> Context.saveToCache(
        ...
        // 🚀 Optimization: Calculate total size and sort files ONCE to avoid O(N^2) file tree walking and high CPU load
        var size = cacheDir.walk().sumOf { it.length() }
        if (size > CACHE_FOLDER_SIZE) {
