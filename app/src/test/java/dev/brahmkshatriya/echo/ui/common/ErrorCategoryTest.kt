...
class ErrorCategoryTest {
    ...
    fun `bare UnknownHost is Network`() {
        assertEquals(ErrorCategory.Network, classify(UnknownHostException("www.example.com")))
    }
    ...
    fun `AppException_Other wrapping UnknownHost is Network`() {
        val chain = IOException(AppException.Other(UnknownHostException("www.example.com"), meta))
